...
public class FileUtils {
     ...
     * @return
     */
    public static void writeFileContent(File file,List<String> objs, String charset) {
        ...
        try {
            ...
            while (iter.hasNext()) {
                outs.write(iter.next().toString().getBytes(charset));
                outs.write("\n".getBytes());
            }
        } catch (Exception ex) {
    ...
    public static File multipartFileToFile(MultipartFile file,String path) throws Exception {
        ...
        if (file.equals("") || file.getSize() <= 0) {
            ...
            toFile = new File(path+DateUtil.getCurrentLongDate()+"_"+file.getOriginalFilename());
            //文件不存在则创建文件
            if (!toFile.exists()){
